package software.dzjz.retrieval.util;
...
public class StringUtil {
    ...
    /**
     * 使用分隔符合并数组为一个字符串。分隔符是 英文逗号
     * @param arr
     * @return
     */
    public static String splitJoint(String[] arr){
        return splitJoint(arr,null);
    }
    /**
     * 使用分隔符合并数组为一个字符串。默认分隔符是 英文逗号
     * @param arr
     * @param delimiter
     * @return
     */
    public static String splitJoint(String[] arr,String delimiter){
     ...
     * @return
     */
    public static String splitJoint(List<String> list){
        return splitJoint(list,null);
    }
    /**
     * 使用分隔符合并数组为一个字符串。默认分隔符是 英文逗号
     * @param list
     ...
     * @return
     */
    public static String splitJoint(List<String> list,String delimiter){
        delimiter=StringUtil.isBlank(delimiter)?",":delimiter;
        StringBuilder res = new StringBuilder();
        for(String str:list){
            res.append(str);
        ...
        if (StringUtil.isBlank(v1)) {
            ...
            if(v1_first.equals("-")){//负号开始
                ...
                if(isNumberic(v1_other)){
                    v1_isnum=true;
                    v1_int=-Integer.parseInt(v1_other);
                }else{
                    v1_int=1;
                }
        ...
        if (StringUtil.isBlank(v2)) {
            ...
            if(v2_first.equals("-")){//负号开始
                ...
                if(isNumberic(v2_other)){
                    v2_isnum=true;
                    v2_int=-Integer.parseInt(v2_other);
                }else{
                    v2_int=2;
                }
            }else{//如果不是负号开始，那就可以直接判断是不是数字字符串了
